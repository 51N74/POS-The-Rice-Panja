...
const OrderForm = ({ onOrderSubmit, tableId }) => { // Accept isAdmin as a prop
  const [menuItems, setMenuItems] = useState([]);
  const [order, setOrder] = useState([]);
  const [total, setTotal] = useState(0);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
  const [selectedCategory, setSelectedCategory] = useState("อาหารจานเดียว"); // Default category
    ...
    try {
      const response = await fetch("/api/order", {
        ...
        body: JSON.stringify({
          ...
          total: total,
          tableId: tableId, // Use the tableId prop here
        }),
      });
  ...
  //   const tableIdString = String(tableId); // Convert to string if necessary
  //   // Redirect to the PaymentPage with query parameters
  //   router.push({
  //     pathname: '/payment',
  //     query: {
  //       tableId: tableId,
  //       orderSummary: JSON.stringify(orderSummary), // Pass order summary as JSON string
  //     },
  //   });
  // };
    ...
    <div className="order-form max-w-6xl mx-auto p-6 bg-yellow-50 rounded-lg shadow-lg">
      <div className="menu-items mb-8">
        <div className="flex space-x-4 mb-4">
          {["อาหารจานเดียว", "กับข้าว", "ของทานเล่น", "เครื่องดื่ม"].map((category) => (
            <button
              key={category}
              onClick={() => setSelectedCategory(category)}
              className={`px-4 py-2 rounded-lg transition duration-300 
                          ${selectedCategory === category ? "bg-orange-500 text-white" : "bg-orange-200 text-gray-700 hover:bg-orange-300"}`}
            >
              {category}
            </button>
          ))}
        ...
        <h2 className="text-3xl font-bold mb-4 text-center text-orange-600">{selectedCategory}</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {menuItems.map((item) => (
            item.category === selectedCategory && (
              <div key={item.id} className="bg-white shadow-lg rounded-lg p-6 flex flex-col border border-orange-200">
                <img
                  src={item.images.length > 0 ? item.images[0].url : "fallback-image-url.jpg"}
                  alt={item.name}
                  className="w-full h-48 object-cover rounded-t-lg mb-4"
                />
                <h3 className="text-xl font-semibold text-orange-600">{item.name}</h3>
                <p className="text-gray-700 mb-2">{item.description}</p>
                <p className="text-lg font-bold mb-4 text-orange-500">${parseFloat(item.price).toFixed(2)}</p>
                <button
      ...
      <div className="current-order mt-8 p-4 bg-white rounded-lg shadow-md border border-orange-200">
        ...
        {order.length === 0 ? (
          ...
          <div>
            {order.map((item) => (
              <div key={item.id} className="flex justify-between items-center mb-2">
                ...
                <div className="flex items-center">
                  <span className="mr-2 font-bold text-orange-600">${(parseFloat(item.price) * item.quantity).toFixed(2)}</span>
                  <button
            ...
            <div className="mt-4">
              <strong className="text-lg">Total: <span className="text-orange-600">${total.toFixed(2)}</span></strong>
            </div>
            <button
              onClick={submitOrder}
